...
int comparaElementos(Personagem a, Personagem b){
    ...
    int Comparison = strcmp(a.eyeColour, b.eyeColour);
    if(Comparison != 0) {
        return Comparison;
    }else {
...
void shellSort(Personagem vetor[]){
    int h = 1;
    do{ h = (h*3) + 1; } while(h < 27);
    do{
        h /= 3;
        for(int cor = 0; cor < h; cor++){
            insercaoPorCor(vetor, cor, h);
        }
    } while(h != 1);
}

void insercaoPorCor(Personagem vetor[], int cor, int h){
    for(int i = (h + cor); i < 27; i += h){
        Personagem tmp = vetor[i];
        int j = i - h;
        while((j >= 0) && comparaElementos(vetor[j], tmp) > 0){
            vetor[j+h] = vetor[j];
            j -= h;
        }
        vetor[j+h] = tmp;
