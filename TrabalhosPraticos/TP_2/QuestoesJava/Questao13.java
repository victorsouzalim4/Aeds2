...
public class Questao13 {
    ...
    private static int compare(Personagem a, Personagem b) {
        int houseComparison = a.getActorName().compareTo(b.getActorName());
    ...
    public static int contaObjetos(Personagem vetor[]){
        int count = 0;
    ...
    public static void intercalar(Personagem vetor[], int esq, int meio, int dir){
        ...
        for(iEsq = 0; iEsq < nEsq; iEsq++){
            arrayEsq[iEsq] = vetor[esq+iEsq].clone(vetor[esq+iEsq]);
        }

        for(iDir = 0; iDir < nDir; iDir++){
            arrayDir[iDir] = vetor[meio + 1 + iDir].clone(vetor[meio + 1 + iDir]);
        }

        for(iEsq = 0, iDir = 0, i = esq; i <= dir; i++){

            if(arrayEsq[iEsq] == null){
                vetor[i] = arrayDir[iDir].clone(arrayDir[iDir]);
                iDir++;
            }
            else if(arrayDir[iDir] == null){
                    vetor[i] = arrayEsq[iEsq].clone(arrayEsq[iEsq]);
                    iEsq++;
            }
            else{
                if(compare(arrayEsq[iEsq], arrayDir[iDir]) < 0){
                    vetor[i] = arrayEsq[iEsq].clone(arrayEsq[iEsq]);
                    iEsq++;
                }
                else{
    ...
    public static void main(String[] args) {
        ...
        SelectionSort(vetor);
        //MergeSort(vetor, 0, contaObjetos(vetor) - 1);
